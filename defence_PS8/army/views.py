...
def generate_pdf(request):
    ...
    # Custom styles
    name_style = ParagraphStyle('NameStyle', parent=normal_style, spaceBefore=12)
    heading_style = ParagraphStyle('HeadingStyle', parent=normal_style, fontSize=14, textColor='rgb(252, 103, 49)', spaceBefore=16)
    total_style = ParagraphStyle('TotalStyle', parent=normal_style, fontSize=14, textColor='rgb(252, 103, 49)', spaceBefore=24)
    table_style = [('GRID', (0, 0), (-1, -1), 1, 'black')]

    # PDF content
    content = []

    # Add name, email, date, and order number
    user = request.user
    name = user.first_name + " " + user.last_name
    email = user.email
    date = datetime.now().strftime("%Y-%m-%d")
    order_no = "012j1gvs356c"  # Replace with actual order number
    
    content.append(Paragraph("Purchase Receipt", title_style))
    content.append(Paragraph("Name: " + name, name_style))
    content.append(Paragraph("Email: " + email, name_style))
    content.append(Paragraph("Date: " + date, name_style))
    content.append(Paragraph("Order No.: " + order_no, name_style))
    content.append(Spacer(1, 12))  # Add space between details and table

    # Add table
    table_data = [["Product Name", "Product Price", "Quantity", "Total"]]
    for item in cart_items:
        table_data.append([
            item.product.name,
...
def buynow(request,id):
    store = Store.objects.get(pk=id)     # cart_items will fetch product of current user, and show product available in the cart of the current user.
    delhivery_charge =150
    gst_rate =0.05
    final_price= delhivery_charge + store.discounted_price
    final_price_with_gst= final_price * (1 + gst_rate) 
    
    address = Customer.objects.filter(user=request.user)

    return render(request, 'buynow.html', {'final_price':final_price,'address':address,'store':store,'final_price_with_gst':final_price_with_gst})
...
def buynow_payment(request,id):
    ...
    return render(request, 'payment.html', {'final_price':final_price, 'final_price_with_gst':final_price_with_gst, 'address':address,'store':store,'paypal':paypal_payment})
...
def buynow_payment_success(request,selected_address_id,id):
    ...
    store = Store.objects.get(pk=id)
    Order(user=user, customer=customer_data, store=store ,quantity=1).save()
   
    return render(request,'buynowpaymentsuccess.html')
